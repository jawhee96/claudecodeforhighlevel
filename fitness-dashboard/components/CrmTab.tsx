...
const LOCS = [
  { key: 'RD' as const, name: 'Red Deer', cls: 'rd' },
  { key: 'SP' as const, name: 'Sherwood Park', cls: 'sp' },
]
  ...
  return (
    <div className="tab-content">
      <div className="kpi-grid">
        ...
        <KpiCard label="Pipeline (open)"     value={fmtK(pipeVal)} />
      ...
      <div className="loc-grid">
        {LOCS.map(loc => {
          ...
          const max = Math.max(locWon.length, locOpen.length, locLost.length) || 1
          const pct = (n: number) => ((n / max) * 100).toFixed(0) + '%'
          return (
            <div key={loc.key} className={`loc-card ${loc.cls}`}>
              <div className="loc-name">{loc.name}</div>
              <div className="funnel-row">
                <span className="funnel-lbl">Won</span>
                <div className="funnel-bar-bg"><div className="funnel-bar" style={{ width: pct(locWon.length), background: 'linear-gradient(90deg,#22c55e,#16a34a)' }} /></div>
                <span className="funnel-count">{locWon.length}</span>
              </div>
              <div className="funnel-row">
                <span className="funnel-lbl">Open</span>
                <div className="funnel-bar-bg"><div className="funnel-bar" style={{ width: pct(locOpen.length), background: 'linear-gradient(90deg,#6366f1,#4f46e5)' }} /></div>
                <span className="funnel-count">{locOpen.length}</span>
              </div>
              <div className="funnel-row">
                <span className="funnel-lbl">Lost</span>
                <div className="funnel-bar-bg"><div className="funnel-bar" style={{ width: pct(locLost.length), background: 'linear-gradient(90deg,#ef4444,#dc2626)' }} /></div>
